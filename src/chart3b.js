...
const graph3b_fill = vl.markGeoshape({stroke: null, opacity: 1})
    ...
    .encode(
        vl.color().fieldQ('pop').scale({scheme: "lightorange", domain: [0,20,60]}).legend({"title": {"value": "Mismanaged Plastic Waste (kg/person/day)", "angle": 90}}),
        vl.tooltip([{field : "country", title: "Country"}, {field: "pop", title: "Mismanaged val"}]),
    )
    .project(vl.projection('equalEarth'))
    .title('Mismanged Plastic Waste Per Person by Country');
